...
//!   - the actor — read from the `Session` the `auth_guard` injected into the
//!     Depot (NEVER a client-supplied field — T-11-14 spoofing mitigation),
...
/// Response-phase audit hoop (ACT-04 / RESEARCH Pattern 5).
///
/// Attached to the protected subtree. Runs the downstream handler FIRST, then
/// (for state-changing methods only) records the actor + method + path + outcome
/// into `console_audit_log`. The actor is read from the Depot `Session`
/// (`auth_guard`-injected) — never from the request body (T-11-14). The insert
/// is best-effort: a DB failure is logged at `warn` and swallowed so auditing
/// can never block or fail the user-facing response.
#[handler]
